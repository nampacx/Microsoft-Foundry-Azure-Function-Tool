...
{
    private const int RunPollingIntervalMs = 5000;
    ...
    private async Task<(List<AgentDefinition> Agents, List<Agents.Models.ToolDefinition> Tools)> ParseDefinitionsAsync()
    {
    ...
    private void ValidateDefinitions(List<AgentDefinition> agents, List<Agents.Models.ToolDefinition> tools)
    {
    ...
    private async Task DownloadOpenApiSpecificationsAsync(List<Agents.Models.ToolDefinition> toolDefinitions)
    {
    ...
    private async Task<PersistentAgent> CreateAgentAsync(
        ...
        List<Agents.Models.ToolDefinition> toolDefinitions,
        Dictionary<string, string>? placeholders = null)
    {
    ...
    private OpenApiToolDefinition? CreateOpenApiTool(Agents.Models.ToolDefinition toolDef)
    {
    ...
    private ConnectedAgentToolDefinition? CreateConnectedAgentTool(Agents.Models.ToolDefinition toolDef)
    {
        ...
        List<AzureToolDefinition> tools)
    {
        ...
        {
            Console.WriteLine("  → Updateing existing agent.");
    ...
    public async Task<ThreadRun> RunAgentAsync(PersistentAgentThread thread, PersistentAgent agent, string userMessage)
    {
        ...
        do
        {
            await Task.Delay(TimeSpan.FromMilliseconds(RunPollingIntervalMs));
